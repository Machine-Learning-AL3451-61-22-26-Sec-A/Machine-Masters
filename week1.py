import streamlit as st import numpy as np import pandas as pd class Node:
def _init_(self, feature=None, value=None, result=None): self.feature = feature

self.value = value self.result = result self.children = {}
class DecisionTreeID3: def _init_(self):
self.root = None
def entropy(self, data):
counts = np.unique(data, return_counts=True) probabilities = counts / len(data)
return -np.sum(probabilities * np.log2(probabilities))
def information_gain(self, data, feature_name, target_name): total_entropy = self.entropy(data[target_name]) unique_values = data[feature_name].unique() weighted_entropy = 0
for value in unique_values:
subset = data[data[feature_name] == value]
weighted_entropy += len(subset) / len(data) * self.entropy(subset[target_name]) return total_entropy - weighted_entropy
...
features = [col for col in data.columns if col != target_name] self.root = self.build_tree(data, features, target_name)
...
# Initialize the DecisionTreeID3 model model = DecisionTreeID3()
